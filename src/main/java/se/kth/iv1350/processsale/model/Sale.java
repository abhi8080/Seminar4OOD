...
public class Sale {
    ...
    /**
     * Retrieves a discount based on which discount the customer is eligible for. 
     * 
    ...
    public Amount retrieveDiscount(String customerID, Discount discountType)
    {
        if(isCustomerEligibleForDiscount(customerID))
          totalCost = discountType.priceAfterDiscount(totalCost, itemsRegistredInSale);
    ...
    private void notifyObservers()
    {
        for(SaleObserver saleObs : this.saleObservers)
            saleObs.newSale(amountPaid);
        
    }
    
    /**
     * All sale observers are added to a list and notified when a new sale is made. 
    ...
    /**
     * Registers an item in the sale and calculates running total. An item is registred in the sale once, and the next time an item with the same identifier
     * is bought, the quantity of the item increases. 
     *
